...
- Dash wallpaper metadata: active slot, crop, fit, and media kind.

Wallpaper image, GIF, and video files are not uploaded yet. The current implementation
syncs wallpaper settings only; media backup needs Firebase Storage or another object
store.
...
2. Add an Android app for the exact package you want to test:
   - Local debug: `com.opendash.app.mui3`
...
7. Add it to the gitignored `local.properties` file:
...
The Google Services plugin is applied only when `google-services.json` contains a
client matching the package being built. If the file is missing or for another package,
OpenDash builds local-only.

## Warnings

- Firebase and Firestore can incur costs. Keep test usage small and review Firebase quotas.
